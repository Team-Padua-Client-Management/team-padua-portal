...
const GAP_AFTER_HEADER = 11;
const GAP_LABEL_TO_INPUT = 7;
const GAP_ROW = 13;
...
export async function generateAdvisorChangeRequestPdf(
  ...
  const drawMixedText = (
    ...
    for (const seg of segments) {
      ...
      for (let w = 0; w < words.length; w++) {
        if (w < words.length - 1 || seg.text.endsWith(' ')) {
  ...
  const drawNameDateTable = (
    ...
    const subY2 = y - 22;
    const fillColor = rgb(0.99, 0.93, 0.72);
    page.drawRectangle({ x: x + colW * 3 + 10, y: subY2 - 2, width: 20, height: 12, color: fillColor });
    page.drawRectangle({ x: x + colW * 3 + 40, y: subY2 - 2, width: 30, height: 12, color: fillColor });
    page.drawRectangle({ x: x + colW * 3 + 80, y: subY2 - 2, width: 35, height: 12, color: fillColor });
    ...
    page.drawLine({ start: { x: x + colW * 3 + 10, y: subY2 - 2 }, end: { x: x + colW * 3 + 30, y: subY2 - 2 }, thickness: 0.5, color: black });
    page.drawLine({ start: { x: x + colW * 3 + 40, y: subY2 - 2 }, end: { x: x + colW * 3 + 70, y: subY2 - 2 }, thickness: 0.5, color: black });
    page.drawLine({ start: { x: x + colW * 3 + 80, y: subY2 - 2 }, end: { x: x + colW * 3 + 115, y: subY2 - 2 }, thickness: 0.5, color: black });
  ...
  const drawDateBox = (
    ...
    const subY2 = y - 15;
    const fillColor = rgb(0.99, 0.93, 0.72);
    page.drawRectangle({ x: x + 10, y: subY2 - 2, width: 20, height: 12, color: fillColor });
    page.drawRectangle({ x: x + 40, y: subY2 - 2, width: 30, height: 12, color: fillColor });
    page.drawRectangle({ x: x + 80, y: subY2 - 2, width: 35, height: 12, color: fillColor });
    ...
    page.drawLine({ start: { x: x + 10, y: subY2 - 2 }, end: { x: x + 30, y: subY2 - 2 }, thickness: 0.5, color: black });
    page.drawLine({ start: { x: x + 40, y: subY2 - 2 }, end: { x: x + 70, y: subY2 - 2 }, thickness: 0.5, color: black });
    page.drawLine({ start: { x: x + 80, y: subY2 - 2 }, end: { x: x + 115, y: subY2 - 2 }, thickness: 0.5, color: black });
  ...
  const item1Y = drawWrappedText(page1, '1. Sun Life of Canada (Philippines), Inc.\\n   Client Support Services\\n   5th Ave. cor Rizal Drive, Bonifacio Global City\\n   Taguig City, Philippines, 1634', marginX, cursorY, colWidth, helveticaFont, 7, 9);

  let rightY = drawWrappedText(page1, '2. Any Financial Store or Client Service Center. Our address and business number are available online at www.sunlife.com.ph. Kindly find one nearest you. Click \'About Us\', and hover to \'Where to find us\'.', colRightX, cursorY, colWidth, helveticaFont, 7, 9);
  rightY -= 4;
  ...
  cursorY = drawWrappedText(page1, 'Specify below the policy/plan/account number(s) to be transferred (incorrect policy/plan/account number(s) will not be processed):', marginX + 15, cursorY, contentWidth - 15, helveticaFont, 8, 10);
  cursorY -= GAP_LABEL_TO_INPUT;
  ...
  cursorY = drawWrappedText(page1, 'B.2 Request will apply to ALL existing client\'s account as of date of request (select the applicable type of account to be transferred):', marginX, cursorY, contentWidth, helveticaBold, 9, 11);
  cursorY -= GAP_LABEL_TO_INPUT;
  ...
  let cursorY2 = page2.getHeight() - 36;

  cursorY2 = drawSectionHeader(page2, 'D', 'New Advisor Information', cursorY2);
  cursorY2 -= GAP_AFTER_HEADER;

  page2.drawText('New Advisor\'s Full Name', { x: marginX, y: cursorY2, size: 9, font: helveticaBold, color: black });
  cursorY2 -= GAP_LABEL_TO_INPUT;

  const colW3 = contentWidth / 3;
  drawBorderedBox(page2, marginX, cursorY2, colW3, 28, 'Last Name', record.new_advisor_last_name);
  drawBorderedBox(page2, marginX + colW3, cursorY2, colW3, 28, 'First Name', record.new_advisor_first_name);
  cursorY2 = drawBorderedBox(page2, marginX + colW3 * 2, cursorY2, colW3, 28, 'Middle Name', record.new_advisor_middle_name);
  cursorY2 -= GAP_SECTION;

  cursorY2 = drawSectionHeader(page2, 'E', 'Signatures', cursorY2);
  cursorY2 -= GAP_AFTER_HEADER;

  cursorY2 = drawWrappedText(page2, 'By signing below, you confirm your understanding and agreement to the following:', marginX, cursorY2, contentWidth, helveticaBold, 8, 10);
  cursorY2 -= GAP_LABEL_TO_INPUT;
  ...
  disclaimers.forEach(d => {
    cursorY2 = drawWrappedText(page2, d, marginX + 15, cursorY2, contentWidth - 15, helveticaFont, 7, 9);
    cursorY2 -= 5;
  });

  cursorY2 -= GAP_LABEL_TO_INPUT;
  cursorY2 = drawWrappedText(page2, 'E.1 Complete Name of Policy Owner/Policy Holder (for Group Insurance)/Plan Holder/Investor', marginX, cursorY2, contentWidth, helveticaBold, 8, 10);
  cursorY2 -= GAP_LABEL_TO_INPUT;
  ...
  cursorY2 = drawNameDateTable(page2, marginX, cursorY2, contentWidth, clientNameParts, 'Date of Signing [e.g. 01-JAN-2019]', sigDateStr, false);
  cursorY2 -= GAP_ROW;

  cursorY2 = drawBorderedBox(page2, marginX, cursorY2, contentWidth, 28, 'Place of Signing', record.place_of_signing);
  cursorY2 -= GAP_ROW;

  page2.drawText('E.2 Accepted:', { x: marginX, y: cursorY2, size: 9, font: helveticaBold, color: black });
  cursorY2 -= GAP_LABEL_TO_INPUT;

  const ownerSigHeight = 48;
  ...
  drawWrappedText(page2, 'Signature of Policy Owner/Policy Holder (for Group Insurance)/Plan Holder/Investor', marginX + 4, cursorY2 - 10, contentWidth - 8, helveticaFont, 7, 8);
  await embedSignature(page2, record.policy_owner_signature, marginX, cursorY2 - ownerSigHeight, contentWidth, ownerSigHeight - 18);
  ...
  drawWrappedText(page2, 'Signature of New Advisor', marginX + 4, cursorY2 - 10, newAdvSigW - 8, helveticaFont, 7, 8);
  await embedSignature(page2, record.new_advisor_signature, marginX, cursorY2 - sigBoxHeight, newAdvSigW, sigBoxHeight - 16);
  ...
  drawWrappedText(
    ...
    marginX + 8, cursorY2 - 11, contentWidth - 16, helveticaBold, 9, 11, white
  );
  ...
  let r1LeftY = tableY - r1pad - 8;
  r1LeftY = drawMixedText(page2, [
    { text: 'Option 1: Via ', bold: false },
    { text: 'Client Portal', bold: true },
    { text: ' (www.sunlife.com.ph)', bold: false }
  ], marginX + r1pad, r1LeftY, col1W - r1pad * 2, 8, helveticaFont, helveticaBold, 10);

  let r1RightY = tableY - r1pad - 8;
  r1RightY = drawMixedText(page2, [
    { text: 'Option 2: Via ', bold: false },
    { text: 'Mobile App', bold: true }
  ], marginX + col1W + r1pad, r1RightY, col2W - r1pad * 2 - 50, 8, helveticaFont, helveticaBold, 10);
  ...
  if (qrRes.ok) {
    ...
    page2.drawImage(qrImage, {
      x: marginX + col1W + col2W - r1pad - 45,
      y: tableY - r1pad - 45,
      width: 45,
      height: 45,
    });
  } else {
    throw new Error('QR code image not found at /images/sunlife-qr-code.png. Please add it.');
  }
  ...
  const row1Bottom = Math.min(r1LeftY, r1RightY - 8) - r1pad;
  const row1Height = tableY - row1Bottom;

  let r2LeftY = row1Bottom - r1pad - 8;
  const bulletIndent = 12;

  const drawNumbered = (page: PDFPage, num: string, segments: any[], x: number, y: number, w: number) => {
    page.drawText(num, { x, y, size: 8, font: helveticaFont, color: black });
    return drawMixedText(page, segments, x + bulletIndent, y, w - bulletIndent, 8, helveticaFont, helveticaBold, 10);
  };
  ...
  let r2RightY = row1Bottom - r1pad - 8;
  ...
  cursorY2 = row2Bottom - GAP_SECTION;
  ...
  cursorY2 = drawWrappedText(page2, f2Text, marginX, cursorY2, contentWidth - 60, helveticaFont, 8, 10);
  ...
  const sectionGHeight = 18 + GAP_AFTER_HEADER + GAP_LABEL_TO_INPUT + 26 + GAP_ROW + 26 + 40;
  let pageG = page2;
  if (cursorY2 - sectionGHeight < 60) {
    pageG = pdfDoc.addPage([595.28, 841.89]);
    cursorY2 = pageG.getHeight() - 40;
  }

  cursorY2 = drawSectionHeader(pageG, 'G', 'For Office Use Only', cursorY2);
  cursorY2 -= GAP_AFTER_HEADER;

  pageG.drawText('Requirements received by', { x: marginX, y: cursorY2, size: 8, font: helveticaBold, color: black });
  cursorY2 -= GAP_LABEL_TO_INPUT;

  drawBorderedBox(pageG, marginX, cursorY2, contentWidth / 2, 26, 'Complete Name of Staff', record.received_by_staff);
  cursorY2 = drawBorderedBox(pageG, marginX + contentWidth / 2, cursorY2, contentWidth / 2, 26, 'Receiving Department/Office', record.receiving_department);
  cursorY2 -= GAP_ROW;
  ...
  drawDateBox(pageG, marginX, cursorY2, contentWidth / 2, 'Date Received [e.g. 01-JAN-2019]', recDateStr);
  drawBorderedBox(pageG, marginX + contentWidth / 2, cursorY2, contentWidth / 2, 26, 'Time Received', record.time_received);

  const allPages = pdfDoc.getPages();
  allPages.forEach((p, idx) => {
    p.drawText('SACR.04.24', { x: marginX, y: 30, size: 8, font: helveticaFont, color: black });
    p.drawText(`Page ${idx + 1} of ${allPages.length}`, { x: marginX + contentWidth - 50, y: 30, size: 8, font: helveticaFont, color: black });
  });
