...
export default function FundWithdrawalPage() {
  ...
  const handleOpenEditor = (record?: WithdrawalRecord) => {
    if (record) {
  ...
  const handleSaveDraftFromEngine = async (engineValues: Record<string, any>) => {
    ...
    try {
      ...
      if (editingRecord) {
        ...
        setSuccess("Fund Withdrawal saved successfully.");
      } else {
        ...
        setSuccess("New Fund Withdrawal request created.");
      }
  ...
  const handleExportPdfFromEngine = async (engineValues: Record<string, any>) => {
    try {
      ...
      a.href = downloadUrl;
      a.download = `Fund_Withdrawal_${clientName || 'Request'}.pdf`;
  ...
  const handleDownloadPdf = async (record: WithdrawalRecord) => {
    try {
      setIsGeneratingPdf(true);
        ...
        <main className="flex-1 p-4 md:p-6 lg:p-8 space-y-6 overflow-y-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl font-bold tracking-tight text-gray-900">Fund Withdrawal</h1>
              <p className="text-sm text-gray-500 mt-1">Manage fund withdrawal requests and generate PDFs using interactive form entry.</p>
            </div>
            <button
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-gray-400 text-sm">No fund withdrawal requests found. Create one to get started.</p>
                        </div>
                      </td>
                    </tr>
                    ...
                    filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50/60 transition-colors duration-150">
                        ...
                        <td className="px-6 py-4 text-gray-900 font-bold">
                          {new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(record.amount || 0)}
                        </td>
                        <td className="px-6 py-4 text-gray-500">
