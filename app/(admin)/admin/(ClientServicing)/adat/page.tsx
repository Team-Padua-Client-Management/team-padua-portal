...
import { Plus, Search, Edit2, Trash2, Download, Filter, Activity } from 'lucide-react';
import AdminHeader from '@/app/components/admin/AdminHeader';
import AdminSidebar from '@/app/components/admin/AdminSidebar';

export default function AdvisorDailyActivityPage() {
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-4 md:p-6 lg:p-8 space-y-6 overflow-y-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-text">Advisor Daily Activity Tracker</h1>
              <p className="text-sm text-text-secondary mt-1">Track daily activities, tasks, and advisory updates.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { label: 'TOTAL LOGGED ACTIVITIES', count: 0, color: 'text-text', icon: Activity, isYellowBorder: true },
              { label: 'TODAY\'S ACTIVITIES', count: 0, color: 'text-orange-500', icon: Filter },
              { label: 'SUCCESSFUL OUTCOMES', count: 0, color: 'text-emerald-500', icon: Download },
            ].map((stat, i) => {
          ...
          <div className="bg-card border border-border/50 shadow-[0_2px_10px_rgb(0,0,0,0.04)] rounded-[20px] overflow-hidden">
            <div className="p-5 border-b border-border flex flex-col md:flex-row gap-4 justify-between items-center bg-card">
              <div className="relative w-full md:w-80">
                ...
                <input
                  type="text"
                  placeholder="Search activity tracker logs..."
                  value={search}
              ...
              <table className="w-full text-left border-collapse text-sm whitespace-nowrap">
                <thead>
                  <tr className="bg-surface-2 border-b border-border">
                    ...
                    <th className="py-2.5 px-3 font-semibold text-text-secondary uppercase text-[10px] tracking-wider">Advisor Name</th>
                    <th className="py-2.5 px-3 font-semibold text-text-secondary uppercase text-[10px] tracking-wider">Activity Log</th>
                    <th className="py-2.5 px-3 font-semibold text-text-secondary uppercase text-[10px] tracking-wider">Status</th>
                    <th className="py-2.5 px-3 font-semibold text-text-secondary uppercase text-[10px] tracking-wider">Log Date</th>
                ...
                <tbody className="divide-y divide-border">
                  <tr>
                    <td colSpan={6} className="py-12 text-center text-text-secondary text-sm">No activity records logged.</td>
                  </tr>
                </tbody>
              </table>
            </div>
